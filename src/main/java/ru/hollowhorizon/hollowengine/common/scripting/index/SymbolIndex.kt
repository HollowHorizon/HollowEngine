            ...
            transaction(db) {
                ...
                for (descriptor in allDescriptors(module)) {
                    ...
                    if (canStoreFqName(descriptorFqn) && (extensionReceiverFqn?.let { canStoreFqName(it) } != false)) {
                        ...
                        HollowEngine.LOGGER.warn("Excluding symbol $descriptorFqn from index since its name is too long")
                    }
                }
    ...
    fun query(prefix: String, receiverType: FqName? = null, limit: Int = 20): List<Symbol> {
        ...
        try {
            ...
            HollowEngine.LOGGER.debug(
                "Query took " + Clock.System.now().minus(start).toDouble(DurationUnit.MILLISECONDS) + " ms"
            )
        }
    }
    ...
    private fun allDescriptors(module: ModuleDescriptor): Sequence<DeclarationDescriptor> = allPackages(module)
        ...
        .flatMap {
            try {
                ...
                HollowEngine.LOGGER.warn("Couldn't query descriptors in package $it")
                emptyList()
            }
        }
...
fun onProgressRender(event: TitleBarCreationEvent.Center) {
    if(SymbolIndex.indexing) event.append {
        Box {
            ...
            Text("Индексация проекта" + ".".repeat(Time.gameTime.toInt() % 4)) {
