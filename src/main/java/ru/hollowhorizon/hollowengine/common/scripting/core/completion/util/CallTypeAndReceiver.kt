...
import ru.hollowhorizon.hollowengine.common.scripting.core.completion.resolve.*
...
fun CallTypeAndReceiver<*, *>.receiverTypesWithIndex(
    ...
    val implicitReceiverValues = resolutionScope.getImplicitReceiversHierarchy().map { it.value }
...
private fun receiverValueTypes(
    ...
    return if (dataFlowValue.isStable || !stableSmartCastsOnly) { // we don't include smart cast receiver types for "unstable" receiver value to mark members grayed
        smartCastManager.getSmartCastVariants(
            receiverValue,
            bindingContext,
            moduleDescriptor,
            dataFlowInfo,
