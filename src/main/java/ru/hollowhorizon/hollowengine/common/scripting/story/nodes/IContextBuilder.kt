...
interface IContextBuilder {
    ...
    infix fun Team.tpTo(pos: SimpleTeleport.() -> Unit) = +SimpleNode {
    ...
    infix fun Team.setGamemode(gameMode: () -> GameMode) = +SimpleNode {
        val mode = gameMode.invoke()

        this@setGamemode.onlineMembers.forEach {
            when(mode) {
                GameMode.SURVIVAL -> it.setGameMode(GameType.SURVIVAL)
                GameMode.CREATIVE -> it.setGameMode(GameType.CREATIVE)
                GameMode.ADVENTURE -> it.setGameMode(GameType.ADVENTURE)
                GameMode.SPECTATOR -> it.setGameMode(GameType.SPECTATOR)
            }
        }
    }
