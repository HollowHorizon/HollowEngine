...
import ru.hollowhorizon.hollowengine.common.project.kt.util.findProjectCommandWithName
...
internal class MavenClassPathResolver private constructor(private val pom: Path) : ClassPathResolver {
    ...
    override val classpath: Set<ClassPathEntry> get() {
        ...
        when {
            artifacts.isEmpty() -> HollowEngine.LOGGER.warn("No artifacts found in {}", pom)
            artifacts.size < 5 -> HollowEngine.LOGGER.info("Found {} in {}", artifacts, pom)
            else -> HollowEngine.LOGGER.info("Found {} artifacts in {}", artifacts.size, pom)
        }
...
private fun findMavenArtifact(a: Artifact, source: Boolean): Path? {
    ...
    return if (Files.exists(result))
        ...
        HollowEngine.LOGGER.warn("Couldn't find {} in {}", a, result)
        null
    }
}
...
private fun runCommand(pom: Path, command: List<String>) {
    ...
    HollowEngine.LOGGER.info("Run {} in {}", command, workingDirectory)
    val (result, errors) = execAndReadStdoutAndStderr(command, workingDirectory)
    HollowEngine.LOGGER.debug(result)
    if ("BUILD FAILURE" in errors) {
        HollowEngine.LOGGER.warn("Maven task failed: {}", errors.lines().firstOrNull())
    }
}
...
private fun mvnCommand(pom: Path): Path {
    return requireNotNull(mvnCommandFromPath ?: findProjectCommandWithName("mvnw", pom)?.also {
        HollowEngine.LOGGER.info("Using mvn wrapper (mvnw) in place of mvn command")
