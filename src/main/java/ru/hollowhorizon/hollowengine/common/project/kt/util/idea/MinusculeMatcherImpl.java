...
final class MinusculeMatcherImpl extends MinusculeMatcher {
     ...
     */
    MinusculeMatcherImpl(String pattern, NameUtil.MatchingCaseSensitivity options, String hardSeparators) {
        myOptions = options;
    ...
    private static FList<TextRange> prependRange(FList<TextRange> ranges, int from, int length) {
        TextRange head = Helper.head(ranges);
    ...
    public int matchingDegree(String name, boolean valueStartCaseMatch, FList<? extends TextRange> fragments) {
        ...
        final TextRange first = Helper.head(fragments);
    ...
    private FList<TextRange> matchBySubstring(String name) {
        ...
        if (infix) {
            ...
            if (index >= 0) {
                return Helper.singleton(TextRange.from(index, patternWithoutWildChar.length - 1));
            }
            return null;
        }
        if (CharArrayUtil.regionMatches(patternWithoutWildChar, 0, patternWithoutWildChar.length, name)) {
            return Helper.singleton(new TextRange(0, patternWithoutWildChar.length));
        }
        return null;
    }
    ...
    private FList<TextRange> matchWildcards(String name,
        ...
        if (patternIndex == myPattern.length) {
            ...
            if (isTrailingSpacePattern() && nameIndex != name.length() && (patternIndex < 2 || !isUpperCaseOrDigit(myPattern[patternIndex - 2]))) {
                ...
                if (spaceIndex >= 0) {
                    return Helper.singleton(TextRange.from(spaceIndex, 1));
                }
                return null;
            }
            return FList.emptyList();
    ...
    private FList<TextRange> findLongestMatchingPrefix(String name,
        ...
        if (patternIndex + fragmentLength >= myPattern.length) {
            return Helper.singleton(TextRange.from(nameIndex, fragmentLength));
        }
