...
import ru.hollowhorizon.hollowengine.common.project.kt.util.execAndReadStdoutAndStderr
import ru.hollowhorizon.hollowengine.common.project.kt.util.findCommandOnPath
import ru.hollowhorizon.hollowengine.common.project.kt.util.isOSWindows
import java.io.File
...
internal class GradleClassPathResolver(private val path: Path, private val includeKotlinDSL: Boolean): ClassPathResolver {
    ...
    override val classpath: Set<ClassPathEntry> get() {
        ...
        return readDependenciesViaGradleCLI(projectDirectory, scripts, tasks)
            .apply { if (isNotEmpty()) HollowEngine.LOGGER.info("Successfully resolved dependencies for '${projectDirectory.fileName}' using Gradle") }
    ...
    override val buildScriptClasspath: Set<Path> get() {
        return if (includeKotlinDSL) {
            ...
            return readDependenciesViaGradleCLI(projectDirectory, scripts, tasks)
                .apply { if (isNotEmpty()) HollowEngine.LOGGER.info("Successfully resolved build script dependencies for '${projectDirectory.fileName}' using Gradle") }
        } else {
            emptySet()
        }
    }
...
private fun gradleScriptToTempFile(scriptName: String, deleteOnExit: Boolean = false): File {
    ...
    HollowEngine.LOGGER.debug("Creating temporary gradle file {}", config.absolutePath)
...
private fun getGradleCommand(workspace: Path): Path {
    ...
    if (Files.isExecutable(wrapper)) {
        ...
        return workspace.parent?.let(::getGradleCommand)
            ...
            ?: error("Could not find 'gradle' on PATH")
    }
}
...
private fun readDependenciesViaGradleCLI(projectDirectory: Path, gradleScripts: List<String>, gradleTasks: List<String>): Set<Path> {
    HollowEngine.LOGGER.info("Resolving dependencies for '{}' through Gradle's CLI using tasks {}...", projectDirectory.fileName, gradleTasks)
    ...
    val dependencies = findGradleCLIDependencies(command, projectDirectory)
        ?.also { HollowEngine.LOGGER.debug("Classpath for task {}", it) }
        .orEmpty()
...
private fun findGradleCLIDependencies(command: List<String>, projectDirectory: Path): Set<Path>? {
    ...
    if ("FAILURE: Build failed" in errors) {
        HollowEngine.LOGGER.warn("Gradle task failed: {}", errors)
    } else {
        for (error in errors.lines()) {
            if ("ERROR: " in error) {
                HollowEngine.LOGGER.warn("Gradle error: {}", error)
            }
        }
    }
...
private fun parseGradleCLIDependencies(output: String): Set<Path>? {
    HollowEngine.LOGGER.debug(output)
