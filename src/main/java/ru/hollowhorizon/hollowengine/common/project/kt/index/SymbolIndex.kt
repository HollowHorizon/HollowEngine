package ru.hollowhorizon.hollowengine.common.project.kt.index
...
class SymbolIndex {
    private val db: Database by lazy {
        Database.connect("jdbc:h2:mem:symbolindex;DB_CLOSE_DELAY=-1", "org.h2.Driver")
    }
        ...
        progressFactory.create("Indexing").thenApplyAsync { progress ->
            try {
                transaction(db) {
                    ...
                    HollowEngine.LOGGER.info("Updated full symbol index in ${finished - started} ms! (${count} symbol(s))")
                }
            } catch (e: Exception) {
    ...
    private fun removeDeclarations(declarations: Sequence<DeclarationDescriptor>) =
        declarations.forEach { declaration ->
            ...
            if (validFqName(descriptorFqn) && (extensionReceiverFqn?.let { validFqName(it) } != false)) {
                ...
                HollowEngine.LOGGER.warn("Excluding symbol {} from index since its name is too long", descriptorFqn.toString())
            }
        }
