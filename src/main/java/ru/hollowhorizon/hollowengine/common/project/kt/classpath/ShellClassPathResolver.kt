...
import ru.hollowhorizon.hollowengine.common.project.kt.util.userHome
import java.io.File
...
internal class ShellClassPathResolver(
    ...
    override val classpath: Set<ClassPathEntry> get() {
        ...
        HollowEngine.LOGGER.info("Run {} in {}", cmd, workingDirectory)
        ...
        fun maybeCreate(file: Path): ShellClassPathResolver? =
            file.takeIf { scriptNames.any { name -> scriptExtensions.any { file.endsWith("$name$it") } } }
                ?.takeIf {
                    ...
                    if (!isExecutable) {
                        HollowEngine.LOGGER.warn("Found classpath script $it that is NOT executable and therefore cannot be used. Perhaps you'd want to chmod +x it?")
                    }
                    isExecutable
                }
