...
interface ClassPathResolver {
    ...
    val classpathOrEmpty: Set<ClassPathEntry> // does not throw exceptions
        get() = try {
            ...
            HollowEngine.LOGGER.warn("Could not resolve classpath using {}: {}", resolverType, e.message)
    ...
    val buildScriptClasspathOrEmpty: Set<Path>
        get() = try {
            ...
            HollowEngine.LOGGER.warn("Could not resolve buildscript classpath using {}: {}", resolverType, e.message)
            emptySet<Path>()
        }
