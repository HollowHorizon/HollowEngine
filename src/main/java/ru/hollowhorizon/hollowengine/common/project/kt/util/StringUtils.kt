...
fun containsCharactersInOrder(candidate: CharSequence, pattern: CharSequence, caseSensitive: Boolean): Boolean {
    ...
    while (iCandidate < candidate.length) {
        var cChar = candidate[iCandidate]
        var pChar = pattern[iPattern]

        if (!caseSensitive) {
            cChar = cChar.lowercaseChar()
            pChar = pChar.lowercaseChar()
        }

        if (iPattern == 0 && iCandidate != 0) return false
