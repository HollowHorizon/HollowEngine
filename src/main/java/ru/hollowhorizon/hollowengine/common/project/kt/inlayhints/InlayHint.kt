package ru.hollowhorizon.hollowengine.common.project.kt.inlayhints
...
import ru.hollowhorizon.hollowengine.common.project.kt.util.preOrderTraversal
...
private fun callableArgNameHints(
    ...
    if (callExpression.getChildOfType<KtLambdaArgument>() != null && false) {
        return
    }
    ...
    val hints = entries.mapNotNull { (t, u) ->
        ...
        if (valueArg != null && !valueArg.isNamed() && valueArg !is KtLambdaArgument && (valueArg.asElement() as? KtValueArgument)?.getArgumentExpression() !is KtReferenceExpression) {
