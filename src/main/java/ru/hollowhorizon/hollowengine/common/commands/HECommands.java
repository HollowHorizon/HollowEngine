...
import static ru.hollowhorizon.hollowengine.common.TextHelperKt.*;
    ...
    public static void register(CommandDispatcher<CommandSourceStack> dispatcher) {
        dispatcher.register(Commands.literal("hollowengine")
                ...
                .then(Commands.literal("reset").executes(context -> {
                    context.getSource().getLevel().getCapability(CapabilityStorage.getCapability(StoryTeamCapability.class)).ifPresent(team -> {
                        try {
                            ...
                            context.getSource().getPlayerOrException().sendMessage(literal("§6[§bHollow Story§6] §bCompleted events reset successfully!"), context.getSource().getPlayerOrException().getUUID());
                        } catch (Exception e) {
                            e.printStackTrace();
                        }
                    });
                ...
                .then(Commands.literal("start-script")
                        .then(Commands.argument("player", EntityArgument.players())
                                .then(Commands.argument("script", StringArgumentType.greedyString()).suggests((context, builder) -> {
                                    ...
                                    context.getSource().getLevel().getCapability(CapabilityStorage.getCapability(StoryTeamCapability.class)).ifPresent(team -> {
                                        try {
                                            Team storyTeam = FTBTeamsAPI.api().getManager().getTeamForPlayerID(player.getUUID()).get();
                ...
                .then(Commands.literal("stop-event").then(Commands.argument("event", StringArgumentType.greedyString()).suggests((context, builder) -> {
                    ...
                    context.getSource().getLevel().getCapability(CapabilityStorage.getCapability(StoryTeamCapability.class)).ifPresent(team -> {
                        try {
                            player.sendMessage(literal("§6[§bHollow Story§6] §bForcely stopped event §6" + event), player.getUUID());
                        } catch (Exception e) {
                            e.printStackTrace();
                            player.sendMessage(literal("§6[§bHollow Story§6] §bFailed to stop event §6" + event), player.getUUID());
                            player.sendMessage(literal("§6[§bHollow Story§6] §bSee logs for more info."), player.getUUID());
                        }
                    });
                    return 1;
                })))
                .then(CommandBuilderKt.buildStringCommand("replay-create", (context, name) -> {
                    ...
                    if (!recorder.isRecording()) {
                        ...
                        player.sendMessage(literal("Started recording"), player.getUUID());
                    } else {
                        player.sendMessage(literal("Already recording"), player.getUUID());
                    }
                    return null;
                }))
                .then(Commands.literal("replay-stop").executes(context -> {
                    ...
                    if (recorder.isRecording()) {
                        ...
                        player.sendMessage(literal("Stopped recording, total frames: " + recorder.getReplay().getPoints().size()), player.getUUID());
                    } else {
                        player.sendMessage(literal("Not recording"), player.getUUID());
                    }
                    return 1;
                }))
                ...
                .then(Commands.literal("show-all-replays").executes(context -> {
                    Player player = context.getSource().getPlayerOrException();
                    player.sendMessage(literal("Найдено записей: " + ReplayStorageCapabilityKt.getACTIVE_REPLAYS().size()), player.getUUID());
