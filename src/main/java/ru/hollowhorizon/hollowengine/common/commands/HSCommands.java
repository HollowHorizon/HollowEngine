...
import ru.hollowhorizon.hollowengine.client.screen.CutsceneWorldEditScreen;
import ru.hollowhorizon.hollowengine.client.screen.NPCBuilderScreen;
                ...
                .then(Commands.literal("start-script")
                        .then(Commands.argument("ignore_sequence", BoolArgumentType.bool())
                                .then(Commands.argument("script", StringArgumentType.greedyString()).suggests((context, builder) -> {
                                    DirectoryManager.INSTANCE.getAllStoryEvents().forEach(file -> builder.suggest(DirectoryManager.INSTANCE.toReadablePath(file)));
                                    return builder.buildFuture();
                                }).executes(context -> {
                                    ...
                                    File script = DirectoryManager.INSTANCE.fromReadablePath(raw);
