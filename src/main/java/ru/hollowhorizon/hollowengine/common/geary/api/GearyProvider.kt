...
private val MCEntity.entityProvider: EntityProvider
    get() = this as EntityProvider
...
private fun create(level: Level, entity: MCEntity): EntityId {
    ...
    entity.entityProvider.`hollowengine$entity` = created.toLong()
...
fun ensureEntity(level: Level, entity: MCEntity): EntityId {
    val provider = entity.entityProvider
    val existing = provider.`hollowengine$entity`
...
fun bindIfInitialized(level: Level, entity: MCEntity): EntityId? {
    val provider = entity.entityProvider
    val current = provider.`hollowengine$entity`
    ...
    provider.`hollowengine$entity` = bound.toLong()
    return bound
}
