...
class TradeMenuGui(val npc: NPCEntity, val editMode: Boolean = false) : ImGuiScreen() {
    ...
    private fun Graphics.drawCurrentTrade() {
    ...
    fun drawSlotOne() {
        Graphics.image("hollowengine:textures/gui/trades/trade_slot_1.png".rl, 26f * scale, 28f * scale)
    }

    fun Graphics.drawSlotTwo(slots: HollowContainer, index: Int) {
        val trade = npc[NPCCapability::class].trades[page * 9 + selectedTrade]
        ...
        *///?} else {
        ...
        val isValid2 = ItemStack.isSameItemSameTags(
            trade.inputs[index * 2 + 1],
            item2
        ) && item2.count >= trade.inputs[index * 2 + 1].count
        //?}
    ...
    private fun drawBackground() {
        Graphics.image(GUI, 420f * scale, 170f * scale)

        Graphics.withFontSize((10 * scale).toInt()) {
            var text = "Инвентарь"
    ...
    private fun Graphics.drawInventory() {
    ...
    private fun Graphics.drawEntity() {
    ...
    private fun Graphics.drawTrades() {
    ...
    private fun Graphics.drawTradeIcon(i: Int) {
        val cursor = ImGui.getCursorPos()
