...
object GraphRenderer {
    val a = ImLong()
    val b = ImLong()
    private val context = NodeEditorContext(NodeEditorConfig().apply { settingsFile = "node_editor.json" }.ptr)
    ...
    fun draw(graph: ScriptGraph) {
        ...
        var color = ImVec4(1f, 1f, 1f, 1f)
        if (pin != null) color = ImGui.colorConvertU32ToFloat4(pin.color)
        ...
        for (link in graph.connections) {
            val color = ImGui.colorConvertU32ToFloat4(link.inputPin.color)
        ...
        if (ImGui.beginPopup("node_editor_context")) {
            ...
            NodesRegistry.entries().keys.toTree().drawMenu()?.let {
                ...
                val canvas = NodeEditor.screenToCanvas(ImGui.getMousePos())
...
object NodeRenderer {
    ...
    fun draw(pinMap: MutableMap<Pin<*>, Int>, id: Int, node: ScriptNode) {
        ...
        node.inputs.forEach { pin ->
            ...
            val pinColor = ImGui.colorConvertU32ToFloat4(pin.color)
        ...
        if (imgui.ImGui.isItemVisible()) {
            ...
            val nodeRect = NodeEditor.getStyle().colors[NodeEditorStyleColor.NodeBorder]
            ...
            if ((headerMax.x > headerMin.x) && (headerMax.y > headerMin.y)) {
                val nodeColor = ImGui.colorConvertU32ToFloat4(node.color)
