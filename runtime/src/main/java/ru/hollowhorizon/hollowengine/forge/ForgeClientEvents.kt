...
import ru.hollowhorizon.hollowengine.common.events.client.render.RenderLevelStageEvent as HollowRenderLevelStageEvent
...
object ForgeClientEvents {
    ...
    private fun onCameraSetup(event: net.minecraftforge.client.event.ViewportEvent.ComputeCameraAngles) {
        val hcEvent = CameraSetupEvent(event.renderer, event.camera, event.partialTick.toFloat(), event.yaw, event.pitch, event.roll)
        hcEvent.post()
    ...
    private fun onRenderLevelStage(event: RenderLevelStageEvent) {
        val stage = when (event.stage) {
            RenderLevelStageEvent.Stage.AFTER_LEVEL -> RenderStage.AFTER_LEVEL
            RenderLevelStageEvent.Stage.AFTER_SKY -> RenderStage.AFTER_SKY
            RenderLevelStageEvent.Stage.AFTER_ENTITIES -> RenderStage.AFTER_ENTITIES
            RenderLevelStageEvent.Stage.AFTER_BLOCK_ENTITIES -> RenderStage.AFTER_BLOCK_ENTITIES
            RenderLevelStageEvent.Stage.AFTER_PARTICLES -> RenderStage.AFTER_PARTICLES
            RenderLevelStageEvent.Stage.AFTER_WEATHER -> RenderStage.AFTER_WEATHER
            RenderLevelStageEvent.Stage.AFTER_SOLID_BLOCKS -> RenderStage.AFTER_SOLID_BLOCKS
            RenderLevelStageEvent.Stage.AFTER_CUTOUT_MIPPED_BLOCKS_BLOCKS -> RenderStage.AFTER_CUTOUT_MIPPED_BLOCKS
            RenderLevelStageEvent.Stage.AFTER_CUTOUT_BLOCKS -> RenderStage.AFTER_CUTOUT_BLOCKS
            RenderLevelStageEvent.Stage.AFTER_TRANSLUCENT_BLOCKS -> RenderStage.AFTER_TRANSLUCENT_BLOCKS
            RenderLevelStageEvent.Stage.AFTER_TRIPWIRE_BLOCKS -> RenderStage.AFTER_TRIPWIRE_BLOCKS
            else -> return
        }

        EventBus.post(
            HollowRenderLevelStageEvent(
                event.levelRenderer,
                event.poseStack,
