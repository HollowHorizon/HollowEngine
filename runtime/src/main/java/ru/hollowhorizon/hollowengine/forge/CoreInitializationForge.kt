...
object CoreInitializationForge {
    init {
        ...
        getSubTypes = { subType ->
            (classes
                ...
                .safeClassesF() + HollowRuntimeManager.annotationIndex
                .getSubTypes(subType.name, isClient)
                .mapNotNull { loadBootstrapOrRuntimeClass(it, HollowCore::class.java.classLoader) }).toSet()
        }
        getAnnotatedClasses = { annotation ->
            (annotations
                ...
                .filter { isClient || !it.clazz.hasAnnotation(ClientOnly::class.java) }
                .safeClasses() + HollowRuntimeManager.annotationIndex
        ...
        getAnnotatedMethods = { annotation ->
            (annotations
                ...
                .filter { isClient || !it.clazz.hasAnnotation(ClientOnly::class.java) }
                .safeMethods() + HollowRuntimeManager.annotationIndex
                .getAnnotatedMethods(annotation.name, isClient)
                .mapNotNull { it.resolve(HollowCore::class.java.classLoader) }).toSet()
        }
    ...
    fun Collection<ModFileScanData.AnnotationData>.safeMethods(): List<Method> = mapNotNull {
        try {
            ...
            loadBootstrapOrRuntimeClass(it.clazz.className, HollowCore::class.java.classLoader)?.declaredMethods
    ...
    private fun Collection<ModFileScanData.AnnotationData>.safeClasses(): List<Class<*>> = mapNotNull {
        try {
            loadBootstrapOrRuntimeClass(it.clazz.className, HollowCore::class.java.classLoader)
        } catch (e: NoClassDefFoundError) {
            HollowCore.LOGGER.warn("Class ${it.clazz.className} cannot be loaded! ${e.message}")
            null
        }
...
private fun Type.hasAnnotation(java: Class<out Annotation>): Boolean {
    return loadBootstrapOrRuntimeClass(className, HollowCore::class.java.classLoader)?.isAnnotationPresent(java) == true
}
*///?}
