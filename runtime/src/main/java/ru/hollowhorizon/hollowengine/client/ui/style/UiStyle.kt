...
fun UiModifierSnapshot.motionDurationMillis(previous: UiModifierSnapshot?): Long {
    val transitionDuration = if (previous == null) {
        ...
        transitions.filter { transition -> previous.changedForTransition(transition.property, this) }
...
data class TransitionProgress(
    ...
    private val completed: Boolean = background == 1f && foreground == 1f && shadow == 1f && opacity == 1f && tint == 1f && filter == 1f && backdropFilter == 1f && translate == 1f && rotate == 1f && scale == 1f && perspective == 1f,
) {
