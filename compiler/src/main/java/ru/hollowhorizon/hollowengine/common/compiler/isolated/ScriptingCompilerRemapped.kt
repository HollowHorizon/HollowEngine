...
class ScriptJvmCompilerRemapped(val hostConfiguration: ScriptingHostConfiguration) : ScriptCompilerProxy {
    ...
    override fun compile(
        ...
        withMessageCollector(script = script) { messageCollector ->
            withScriptCompilationCache(script, scriptCompilationConfiguration, messageCollector) {
                ...
                val context = Hooks.createCompilationContextFromEnvironment(initialConfiguration, environment, messageCollector)
...
private fun compileImpl(
    ...
    val (sourceFiles, sourceDependencies) = Hooks.collectRefinedSourcesAndUpdateEnvironment(
        context,
        mainKtFile,
    ...
    return doCompileWithK2(context, script, sourceFiles, sourceDependencies, messageCollector, getScriptConfiguration)
}
